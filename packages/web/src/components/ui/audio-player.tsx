...
function formatTime(seconds: number) {
  // Guard non-finite input (a webm/opus recording reports duration=Infinity until scanned) so the
  // readout degrades to 0:00 instead of "Infinity:NaN:NaN".
  ...
  return (
    <AudioPlayerContext.Provider value={api as AudioPlayerApi<unknown>}>
      <AudioPlayerTimeContext.Provider value={time}>
        {/* No crossOrigin: audio is played, never sampled (no Web Audio analyser), so cross-origin
            content (the content worker) plays without needing CORS headers. */}
