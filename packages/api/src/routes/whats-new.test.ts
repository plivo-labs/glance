...
// CATALOG-COUPLED: the unreadCount literals below (`10`) = the number of releases dated after MID.
...
// comment-on-the-page 07-30T16, api-keys 07-31T10 → 10 unread.)
...
describe('C7 route.get.exactJSON — exact ordered items, unreadCount, throughDate', () => {
  test('mid watermark → exact slugs/dates/bodies + unreadCount 10 + throughDate===newest', async () => {
...
describe('B3 relogin.noReset — the existing-user branch must not clear an existing watermark', () => {
  test('re-running findOrCreateUser leaves a mid watermark + unreadCount unchanged', async () => {
    ...
    expect(((await res.json()) as { unreadCount: number }).unreadCount).toBe(10)
  })
})
