...
async function serve(c: Ctx, spaceSlug: string, siteSlug: string, rest: string, userId: string | null): Promise<Response> {
  ...
  if (rangeable && rangeHeader) {
    const ifRange = c.req.header('if-range')
    // A stale If-Range precondition (client's cached range predates this ETag) means the
    // Range no longer applies — fall through to a full 200 body instead of a mismatched slice.
    if (!ifRange || ifRange === object.httpEtag) {
      ...
      // 'none' (no/malformed spec) or 'multi' (comma-separated — legit per RFC 7233 §3.1 to
      // just ignore and serve the whole thing; we don't implement multipart/byteranges) both
      // fall through to the full body below.
    }
  }
